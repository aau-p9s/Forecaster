...
@api.route("/train/<string:serviceId>/<int:forecastHorizon>")
class Train(Resource):
    @api.doc(params={"serviceId":"your-service-id"}, responses={200:"ok", 202:"working...", 500:"Something ML died!!!!"})
    def post(self, serviceId, forecastHorizon:int=12):
        ...
        if not serviceId in trainers:
            ...
            data = historical[0].data
            trainer = Trainer(models, serviceId, data, forecastHorizon, model_repository, forecast_repository)
            trainers[serviceId] = {
                "trainer": trainer,
