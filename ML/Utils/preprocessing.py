...
def remove_outliers_zscore(timeseries: TimeSeries | None, threshold=3):
    if timeseries is None:
        raise ValueError("TimeSeries is None.")
...
def decompose_and_detrend(series: TimeSeries):
    ...
    result = stl.fit()
    detrended = series.with_values(result.resid)
    return detrended
...
def run_transformer_pipeline(
    ...
    without_outliers_timeseries = remove_outliers_zscore(non_negative_timeseries, outlier_thresh)
    ...
    denoised_timeseries = denoiser(handled_missing_timeseries)

    print(f"Scaling data")
    final_timeseries, transformer = scaler(denoised_timeseries)
