...
# Count occurrences of each unique date-time (to the second)
date_time_counts = df["date_time"].value_counts().reset_index()
...
# Create a complete range from the minimum to the maximum date-time in the data
...
# Save to a CSV file
full_date_time_counts.to_csv("agg_second.csv", index=False)

print("Date-time counts with filled missing dates saved to 'agg_second.csv'.")
